...
Use `TableArgument.forArgument(name).type(...)` to configure the input table schema:
...
void testMultiTable() throws Exception {
  try (ProcessTableFunctionTestHarness<Row> harness =
    ...
    .withTableArgument(
        TableArgument.forArgument("left")
            .type(DataTypes.of("ROW<id INT, name STRING>"))
            .partitionBy("id")
            .build())
    .withTableArgument(
        TableArgument.forArgument("right")
