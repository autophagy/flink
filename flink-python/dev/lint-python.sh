...
function tox_check() {
    ...
    if [[ ${BUILD_REASON} = 'IndividualCI' ]]; then
        ...
        $UV_PATH run --group tox tox -vv -c $FLINK_PYTHON_DIR/tox.ini -e ${LATEST_PYTHON} --recreate 2>&1 | tee -a $LOG_FILE
    else
        # Only run random selected python version in nightly CI.
        ENV_LIST_STRING=`$UV_PATH run --group tox tox -l -c $FLINK_PYTHON_DIR/tox.ini`
        _OLD_IFS=$IFS
        IFS=$'\n'
...
function flake8_check() {
    ...
    set -o pipefail
    ($UV_PATH run --group flake8 flake8 --config=tox.ini $PYTHON_SOURCE) 2>&1 | tee -a $LOG_FILE
...
function sphinx_check() {
    # Set SPHINXBUILD to use uv run with sphinx group
...
function mypy_check() {
    ...
    ($UV_PATH run --group mypy mypy --install-types --non-interactive --config-file tox.ini) 2>&1 | tee -a ${LOG_FILE}
