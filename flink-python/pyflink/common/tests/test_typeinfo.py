...
class TypeInfoTests(PyFlinkTestCase):
    def test_row_type(self):
        self.assertEqual(
            RowTypeInfo([Types.STRING(), Types.STRING()]).get_field_names(), ["f0", "f1"]
        )
        self.assertEqual(
            RowTypeInfo([Types.STRING(), Types.STRING()], ["a", "b"]).get_field_names(), ["a", "b"]
        )

        self.assertEqual(
            RowTypeInfo([Types.STRING(), Types.STRING()], ["a", "b"])
            == RowTypeInfo([Types.STRING(), Types.STRING()], ["a", "b"]),
            True,
        )
        self.assertEqual(
            RowTypeInfo([Types.STRING(), Types.STRING()], ["a", "b"])
            == RowTypeInfo([Types.STRING(), Types.INT()], ["a", "b"]),
            False,
        )
        self.assertEqual(
            RowTypeInfo([Types.STRING(), Types.STRING()], ["a", "b"]).__str__(),
        ...
        self.assertEqual(
            Types.ROW_NAMED(["a", "b"], [Types.STRING(), Types.STRING()]).get_field_names(),
            ["a", "b"],
            True,
        )

        self.assertEqual(
            Types.ROW_NAMED(["a", "b"], [Types.STRING(), Types.STRING()]).get_field_types(),
    ...
    def test_tuple_type(self):
        ...
        self.assertEqual(
            TupleTypeInfo([Types.STRING(), Types.INT()]).__str__(), "TupleTypeInfo(String, Integer)"
        )
        ...
        self.assertEqual(
            Types.TUPLE([Types.STRING(), Types.INT()]), TupleTypeInfo([Types.STRING(), Types.INT()])
        )
    ...
    def test_from_java_type(self):
        ...
        self.assertEqual(
            basic_int_type_info, _from_java_type(basic_int_type_info.get_java_type_info())
        )

        basic_short_type_info = Types.SHORT()
        self.assertEqual(
            basic_short_type_info, _from_java_type(basic_short_type_info.get_java_type_info())
        )

        basic_long_type_info = Types.LONG()
        self.assertEqual(
            basic_long_type_info, _from_java_type(basic_long_type_info.get_java_type_info())
        )

        basic_float_type_info = Types.FLOAT()
        self.assertEqual(
            basic_float_type_info, _from_java_type(basic_float_type_info.get_java_type_info())
        )

        basic_double_type_info = Types.DOUBLE()
        self.assertEqual(
            basic_double_type_info, _from_java_type(basic_double_type_info.get_java_type_info())
        )

        basic_char_type_info = Types.CHAR()
        self.assertEqual(
            basic_char_type_info, _from_java_type(basic_char_type_info.get_java_type_info())
        )

        basic_byte_type_info = Types.BYTE()
        self.assertEqual(
            basic_byte_type_info, _from_java_type(basic_byte_type_info.get_java_type_info())
        )

        basic_big_int_type_info = Types.BIG_INT()
        self.assertEqual(
            basic_big_int_type_info, _from_java_type(basic_big_int_type_info.get_java_type_info())
        )
        ...
        self.assertEqual(
            basic_sql_date_type_info, _from_java_type(basic_sql_date_type_info.get_java_type_info())
        )

        basic_sql_time_type_info = Types.SQL_TIME()
        self.assertEqual(
            basic_sql_time_type_info, _from_java_type(basic_sql_time_type_info.get_java_type_info())
        )

        basic_sql_timestamp_type_info = Types.SQL_TIMESTAMP()
        self.assertEqual(
            basic_sql_timestamp_type_info,
