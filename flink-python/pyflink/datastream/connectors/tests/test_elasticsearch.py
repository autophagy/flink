...
@unittest.skip("Disable due to Elasticsearch connector need to release a new version 2.0")
class FlinkElasticsearch7Test(PyFlinkStreamingTestCase):
    def test_es_sink(self):
        ...
        self.assertTrue(
            is_instance_of(
                j_emitter, "org.apache.flink.connector.elasticsearch.sink.MapElasticsearchEmitter"
            )
        )
        self.assertEqual(
            get_field_value(es_sink.get_java_function(), "hosts")[0].toString(),
            "http://localhost:9200",
        )
        self.assertEqual(
            get_field_value(es_sink.get_java_function(), "deliveryGuarantee").toString(),
            "at-least-once",
        )
