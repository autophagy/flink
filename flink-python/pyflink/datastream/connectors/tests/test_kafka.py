...
class KafkaSourceTests(PyFlinkStreamingTestCase):
    ...
    def test_legacy_kafka_connector(self):
        ...
        self.assertTrue(
            get_field_value(flink_kafka_consumer.get_java_function(), "enableCommitOnCheckpoints")
        )
        ...
        self.assertTrue(
            j_start_up_mode.equals(
                get_gateway().jvm.org.apache.flink.streaming.connectors.kafka.config.StartupMode.EARLIEST
            )
        )
        j_topic_desc = get_field_value(flink_kafka_consumer.get_java_function(), "topicsDescriptor")
        ...
        self.assertFalse(
            get_field_value(flink_kafka_producer.get_java_function(), "writeTimestampToKafka")
        )
    ...
    def test_set_starting_offsets(self):
        ...
        self._check_reader_handled_offsets_initializer(
            _build_source(KafkaOffsetsInitializer.earliest()), -2, KafkaOffsetResetStrategy.EARLIEST
        )
        self._check_reader_handled_offsets_initializer(
        ...
        self._check_specified_offsets_initializer(
            _build_source(
                KafkaOffsetsInitializer.offsets(specified_offsets, KafkaOffsetResetStrategy.LATEST)
            ),
            specified_offsets,
            KafkaOffsetResetStrategy.LATEST,
        )
    ...
    def _check_reader_handled_offsets_initializer(
        ...
        self.assertTrue(offset_reset_strategy.equals(reset_strategy._to_j_offset_reset_strategy()))

    def _check_timestamp_offsets_initializer(
        self, source: KafkaSource, timestamp: int, is_start: bool = True
    ):
        if is_start:
            field_name = "startingOffsetsInitializer"
        else:
...
class KafkaRecordSerializationSchemaTests(PyFlinkTestCase):
    ...
    def test_set_key_serialization_schema(self):
        def _check_key_serialization_schema(key_serialization_schema, expected_class):
            ...
            schema_field = get_field_value(
                serialization_schema._j_serialization_schema, "keySerializationSchema"
            )
    ...
    def test_set_value_serialization_schema(self):
        def _check_value_serialization_schema(value_serialization_schema, expected_class):
            ...
            schema_field = get_field_value(
                serialization_schema._j_serialization_schema, "valueSerializationSchema"
            )
            self.assertIsNotNone(schema_field)
            self.assertEqual(schema_field.getClass().getCanonicalName(), expected_class)
    ...
    def _check_serialization_schema_implementations(check_function):
        ...
        check_function(
            SimpleStringSchema(), "org.apache.flink.api.common.serialization.SimpleStringSchema"
        )
        check_function(
            ByteArraySchema(), "org.apache.flink.api.common.serialization.ByteArraySchema"
        )
