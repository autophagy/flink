...
@unittest.skip(
    "Disabled due to pulsar is still using the deprecated api, and it should be removed "
...
class FlinkPulsarTest(PyFlinkUTTestCase):
    def test_pulsar_source(self):
        ...
        self.assertEqual(configuration.get(test_option._j_config_option), True)
        self.assertEqual(
            configuration.get(
    ...
    def test_source_set_topics_with_list(self):
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            "http://localhost:8080"
        ).set_topics(["ada", "beta"]).set_subscription_name("ff").set_deserialization_schema(
            SimpleStringSchema()
        ).build()
    ...
    def test_source_set_topics_pattern(self):
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            "http://localhost:8080"
        ).set_topic_pattern("ada.*").set_subscription_name("ff").set_deserialization_schema(
            SimpleStringSchema()
        ).build()
    ...
    def test_stop_cursor_publish_time(self):
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            "http://localhost:8080"
        ).set_topics("ada").set_subscription_name("ff").set_deserialization_schema(
            SimpleStringSchema()
        ).set_start_cursor(StartCursor.from_publish_time(2)).set_bounded_stop_cursor(
            StopCursor.at_publish_time(14)
        ).set_bounded_stop_cursor(StopCursor.after_publish_time(24)).build()
    ...
    def test_stop_cursor_event_time(self):
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            ...
            SimpleStringSchema()
        ).set_bounded_stop_cursor(StopCursor.after_event_time(14)).set_bounded_stop_cursor(
            StopCursor.at_event_time(24)
        ).build()
    ...
    def test_set_range_generator(self):
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            ...
            SimpleStringSchema()
        ).set_range_generator(RangeGenerator.full()).build()
        ...
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            ...
            RangeGenerator.fixed_key(keys="k", key_bytes=bytearray(b"abc"))
        ).build()
    ...
    def test_set_authentication(self):
        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            ...
            SimpleStringSchema()
        ).set_authentication("test.class", "key1:val1,key2:val2").build()

        PulsarSource.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            "http://localhost:8080"
        ).set_topics(["ada", "beta"]).set_subscription_name("ff").set_deserialization_schema(
            SimpleStringSchema()
        ).set_authentication("test.class", {"k1": "v1", "k2": "v2"}).build()

    def test_pulsar_sink(self):
        ds = self.env.from_collection(
            [("ab", 1), ("bdc", 2), ("cfgs", 3), ("deeefg", 4)],
    ...
    def test_sink_set_topics_with_list(self):
        PulsarSink.builder().set_service_url("pulsar://localhost:6650").set_admin_url(
            "http://localhost:8080"
        ).set_topics(["ada", "beta"]).set_serialization_schema(SimpleStringSchema()).build()
