...
class FileSourceAvroInputFormatTests(PyFlinkUTTestCase):
    ...
    def _create_avro_file(self, schema: AvroSchema, records: list):
        ...
        j_datum_writer = (
            jvm.org.apache.flink.avro.shaded.org.apache.avro.generic.GenericDatumWriter()
        )
        j_file_writer = jvm.org.apache.flink.avro.shaded.org.apache.avro.file.DataFileWriter(
            j_datum_writer
        )
...
def _create_basic_avro_schema_and_records() -> Tuple[AvroSchema, List[JavaObject]]:
    ...
    records = [
        _create_basic_avro_record(schema, True, 0, 1, 2, 3, "s1"),
...
def _create_enum_avro_schema_and_records() -> Tuple[AvroSchema, List[JavaObject]]:
    ...
    records = [
        _create_enum_avro_record(schema, "SPADES"),
...
def _create_union_avro_schema_and_records() -> Tuple[AvroSchema, List[JavaObject]]:
    ...
    records = [
        _create_union_avro_record(schema, 1),
        _create_union_avro_record(schema, 2.0),
...
def _create_array_avro_schema_and_records() -> Tuple[AvroSchema, List[JavaObject]]:
    ...
    records = [
        _create_array_avro_record(schema, [(1, 2.0), (3, 4.0)]),
        _create_array_avro_record(schema, [(5, 6.0), (7, 8.0)]),
    ]
    return schema, records
...
def _create_array_avro_schema_and_py_objects() -> Tuple[AvroSchema, List[dict]]:
    schema = AvroSchema.parse_string(ARRAY_SCHEMA)
    records = [
        {"array": [{"int": 1, "double": 2.0}, {"int": 3, "double": 4.0}]},
        {"array": [{"int": 5, "double": 6.0}, {"int": 7, "double": 8.0}]},
    ]
    return schema, records
...
def _check_array_avro_schema_results(test, results):
    ...
    test.assertAlmostEqual(result1["array"][0]["double"], 2.0, delta=1e-3)
    test.assertEqual(result1["array"][1]["int"], 3)
    test.assertAlmostEqual(result1["array"][1]["double"], 4.0, delta=1e-3)
    test.assertEqual(result2["array"][0]["int"], 5)
    test.assertAlmostEqual(result2["array"][0]["double"], 6.0, delta=1e-3)
    test.assertEqual(result2["array"][1]["int"], 7)
    test.assertAlmostEqual(result2["array"][1]["double"], 8.0, delta=1e-3)
...
def _create_map_avro_schema_and_records() -> Tuple[AvroSchema, List[JavaObject]]:
    ...
    records = [
        _create_map_avro_record(schema, {"a": 1, "b": 2}),
