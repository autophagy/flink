...
from pyflink.datastream.formats.csv import (
    CsvSchema,
    CsvReaderFormat,
    CsvBulkWriters,
...
class FileSourceCsvReaderFormatTests(object):
    ...
    def _build_csv_job(self, schema, lines):
        ...
        ds.map(PassThroughMapFunction(), output_type=Types.PICKLED_BYTE_ARRAY()).add_sink(
            self.test_sink
        )


class ProcessFileSourceCsvReaderFormatTests(
    FileSourceCsvReaderFormatTests, PyFlinkStreamingTestCase
):
    pass


class EmbeddedFileSourceCsvReaderFormatTests(
    FileSourceCsvReaderFormatTests, PyFlinkStreamingTestCase
):
    def setUp(self):
...
class JsonSerializationSchemaTests(PyFlinkTestCase):
    def test_csv_row_serialization_schema(self):
        ...
        def field_assertion(field_info, csv_value, value, field_delimiter):
            ...
            csv_row_serialization_schema = (
                CsvRowSerializationSchema.Builder(row_info)
                .set_escape_character("*")
                .set_quote_character("'")
                .set_array_element_delimiter(":")
                .set_field_delimiter(";")
                .build()
            )
            csv_row_deserialization_schema = (
                CsvRowDeserializationSchema.Builder(row_info)
                .set_escape_character("*")
                .set_quote_character("'")
                .set_array_element_delimiter(":")
                .set_field_delimiter(";")
            ...
            j_deserialized_row = (
                csv_row_deserialization_schema._j_deserialization_schema.deserialize(
        ...
        field_assertion(
            Types.ROW([Types.STRING(), Types.STRING()]), "'1:hello world'", test_j_row, ";"
        )
...
def _create_csv_allow_comments_schema_and_lines() -> Tuple[CsvSchema, List[str]]:
    schema = CsvSchema.builder().add_string_column("string").set_allow_comments().build()
    lines = [
        "a\n",
        "# this is comment\n",
        "b\n",
