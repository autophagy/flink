...
from pyflink.datastream.formats.tests.test_parquet import (
    ...
    _check_parquet_array_results,
    _create_parquet_map_row_and_data,
...
@unittest.skipIf(
    os.environ.get("HADOOP_CLASSPATH") is None, "Some Hadoop lib is needed for Orc format tests"
)
class FileSinkOrcBulkWriterTests(PyFlinkStreamingTestCase):
    ...
    def _build_orc_job(self, row_type: RowType, row_type_info: RowTypeInfo, data: List[Row]):
        ...
        ds = DataStream(
            self.env._j_stream_execution_environment.fromCollection(
                j_list, row_type_info.get_java_type_info()
            )
        )
        ds.sink_to(sink)
