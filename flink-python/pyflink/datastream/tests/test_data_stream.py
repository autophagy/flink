...
class DataStreamTests(object):
    ...
    def test_partition_custom(self):
        ...
        partitioned_stream = (
            ds.map(lambda x: x, output_type=Types.ROW([Types.STRING(), Types.INT()]))
            .set_parallelism(4)
            .partition_custom(my_partitioner, lambda x: x[1])
        )

        JPartitionCustomTestMapFunction = (
            get_gateway().jvm.org.apache.flink.python.util.PartitionCustomTestMapFunction
        )
        test_map_stream = DataStream(
            partitioned_stream._j_data_stream.map(JPartitionCustomTestMapFunction())
        )
    ...
    def test_keyed_process_function_with_state(self):
        ...
        class MyProcessFunction(KeyedProcessFunction):
            ...
            def process_element(self, value, ctx):
                ...
                yield (
                    "current key: {}, current value state: {}, current list state: {}, "
                    "current map state: {}, current value: {}".format(
                        str(current_key),
                        str(current_value),
                        str(current_list),
                        map_entries_string,
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[1], key_type=Types.STRING()
    ...
    def test_reducing_state(self):
        ...
        class MyProcessFunction(KeyedProcessFunction):
            ...
            def open(self, runtime_context: RuntimeContext):
                self.reducing_state = runtime_context.get_reducing_state(
                    ReducingStateDescriptor("reducing_state", lambda i, i2: i + i2, Types.INT())
                )
        ...
        data_stream.key_by(lambda x: x[1], key_type=Types.STRING()).process(
            MyProcessFunction(), output_type=Types.TUPLE([Types.INT(), Types.STRING()])
        ).add_sink(self.test_sink)
    ...
    def test_keyed_co_process(self):
        ...
        ds1 = self.env.from_collection(
            [("a", 1), ("b", 2), ("c", 3)], type_info=Types.ROW([Types.STRING(), Types.INT()])
        )
        ds2 = self.env.from_collection(
            [("b", 2), ("c", 3), ("d", 4)], type_info=Types.ROW([Types.STRING(), Types.INT()])
        )
        ...
        ds1.connect(ds2).key_by(lambda x: x[0], lambda x: x[0]).process(
            MyKeyedCoProcessFunction()
        ).map(lambda x: Row(x[0], x[1] + 1)).add_sink(self.test_sink)
    ...
    def test_keyed_co_broadcast_process(self):
        ...
        ds_broadcast: DataStream = self.env.from_collection(
            [(0, "1603708215000", "a"), (1, "1603708215000", "b")],
            type_info=Types.ROW([Types.INT(), Types.STRING(), Types.STRING()]),
        )
        watermark_strategy = WatermarkStrategy.for_monotonous_timestamps().with_timestamp_assigner(
            SecondColumnTimestampAssigner()
    ...
    def test_process_side_output(self):
        ...
        ds = self.env.from_collection(
            [("a", 0), ("b", 1), ("c", 2)], type_info=Types.ROW([Types.STRING(), Types.INT()])
        )
    ...
    def test_side_output_chained_with_upstream_operator(self):
        ...
        ds2 = ds.map(lambda e: (e[0], e[1] + 1)).process(
            MyProcessFunction(), output_type=Types.STRING()
        )
        main_sink = DataStreamTestSinkFunction()
        ds2.add_sink(main_sink)
        side_sink = DataStreamTestSinkFunction()
    ...
    def test_co_process_side_output(self):
        ...
        ds2 = self.env.from_collection(
            [(3, "c"), (1, "a"), (0, "d")], type_info=Types.ROW([Types.INT(), Types.STRING()])
        )
    ...
    def test_co_broadcast_side_output(self):
        ...
        ds_broadcast = self.env.from_collection(
            [(3, "d"), (4, "f")], type_info=Types.ROW([Types.INT(), Types.STRING()])
        )
        map_state_desc = MapStateDescriptor(
    ...
    def test_keyed_co_process_side_output(self):
        ...
        ds3 = (
            ds1.key_by(lambda e: e[0])
            .connect(ds2.key_by(lambda e: e[1]))
            .process(
                MyKeyedCoProcessFunction(), output_type=Types.TUPLE([Types.STRING(), Types.INT()])
            )
        )
        main_sink = DataStreamTestSinkFunction()
    ...
    def test_keyed_co_broadcast_side_output(self):
        ...
        ds_broadcast = self.env.from_collection(
            [(5, "c"), (6, "d")], type_info=Types.ROW([Types.INT(), Types.STRING()])
        )
        map_state_desc = MapStateDescriptor(
            "dummy", key_type_info=Types.INT(), value_type_info=Types.STRING()
        )
        ds = (
            ...
            .connect(ds_broadcast.broadcast(map_state_desc))
            .process(MyKeyedBroadcastProcessFunction(), output_type=Types.STRING())
        )
        side_sink = DataStreamTestSinkFunction()
        ds.get_side_output(tag).add_sink(side_sink)
        ds.add_sink(self.test_sink)
    ...
    def test_side_output_tag_reusing(self):
        ...
        side2_sink = DataStreamTestSinkFunction()
        ds.map(lambda i: i * 2).process(MyProcessFunction()).get_side_output(tag).add_sink(
            side2_sink
        )
...
class ProcessDataStreamTests(DataStreamTests):
    ...
    def test_basic_co_operations_with_output_type(self):
        ...
        (
            ds_1.connect(ds_2)
            .flat_map(MyCoFlatMapFunction(), output_type=Types.INT())
            .connect(ds_2)
            .map(MyCoMapFunction(), output_type=Types.INT())
            .add_sink(self.test_sink)
        )
    ...
    def test_keyed_co_map(self):
        ...
        ds1.connect(ds2).key_by(MyKeySelector(), MyKeySelector(), key_type=Types.INT()).map(
            AssertKeyCoMapFunction()
        ).map(lambda x: (x[0], x[1] + 1)).add_sink(self.test_sink)
    ...
    def test_keyed_co_flat_map(self):
        ds1 = self.env.from_collection(
            [(1, 1), (2, 2), (3, 3)], type_info=Types.ROW([Types.INT(), Types.INT()])
        )
        ds2 = self.env.from_collection(
            [("a", "a"), ("b", "b"), ("c", "c"), ("a", "a")],
            type_info=Types.ROW([Types.STRING(), Types.STRING()]),
        )
        ds1.connect(ds2).key_by(lambda x: 1, lambda x: 1).flat_map(
            MyRichCoFlatMapFunction(), output_type=Types.STRING()
        ).filter(lambda x: x != "4").add_sink(self.test_sink)
    ...
    def test_keyed_map(self):
        ...
        keyed_stream.map(AssertKeyMapFunction()).map(lambda x: (x[0], x[1] + 1)).add_sink(
            self.test_sink
        )
    ...
    def test_keyed_flat_map(self):
        ...
        keyed_stream.flat_map(AssertKeyMapFunction()).map(lambda x: (x[0], x[1] + 1)).add_sink(
            self.test_sink
        )
    ...
    def test_keyed_filter(self):
        ...
        keyed_stream.filter(AssertKeyFilterFunction()).filter(lambda x: x[1] > 0).add_sink(
            self.test_sink
        )
        self.env.execute("key_by_test")
        results = self.test_sink.get_results(False)
    ...
    def test_multi_key_by(self):
        ...
        ds.key_by(MyKeySelector(), key_type=Types.INT()).key_by(lambda x: x[0]).add_sink(
            self.test_sink
        )
    ...
    def test_collection_type_info(self):
        ...
        results = self.test_sink.get_results()
        expected = [
            "+I[1, [1.1, 1.2, 1.3], [null, hi, flink], 2021-01-09, 12:00:00,"
    ...
    def test_process_function(self):
        ...
        class MyProcessFunction(ProcessFunction):
            def process_element(self, value, ctx):
                ...
                yield "current timestamp: {}, current_value: {}".format(
                    str(current_timestamp), str(value)
                )

        watermark_strategy = WatermarkStrategy.for_monotonous_timestamps().with_timestamp_assigner(
            SecondColumnTimestampAssigner()
        )
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).process(
            MyProcessFunction(), output_type=Types.STRING()
        ).add_sink(self.test_sink)
        self.env.execute("test process function")
        results = self.test_sink.get_results()
        expected = [
            "current timestamp: 1603708211000, " "current_value: Row(f0=1, f1='1603708211000')",
            "current timestamp: 1603708224000, " "current_value: Row(f0=2, f1='1603708224000')",
            "current timestamp: 1603708226000, " "current_value: Row(f0=3, f1='1603708226000')",
            "current timestamp: 1603708289000, " "current_value: Row(f0=4, f1='1603708289000')",
        ]
        self.assert_equals_sorted(expected, results)


class ProcessDataStreamStreamingTests(
    ...
    def test_keyed_sum(self):
        ...
        ds.key_by(lambda x: x[0]).sum("v2").key_by(lambda x: x[0]).sum(1).map(
            lambda x: (x[1], x[0]), output_type=Types.TUPLE([Types.INT(), Types.INT()])
        ).key_by(lambda x: x[1]).sum().add_sink(self.test_sink)
    ...
    def test_keyed_min_by_and_max(self):
        self.env.set_parallelism(1)
        ds = self.env.from_collection(
            [("a", 3, 0), ("a", 1, 1), ("b", 5, 0), ("b", 3, 1)],
            type_info=Types.ROW_NAMED(
                ["v1", "v2", "v3"], [Types.STRING(), Types.INT(), Types.INT()]
            ),
        )
        ...
        ds.key_by(lambda x: x[0]).min_by("v2").map(
            lambda x: (x[0], x[1], x[2]),
            output_type=Types.TUPLE([Types.STRING(), Types.INT(), Types.INT()]),
        ).key_by(lambda x: x[2]).max(1).key_by(lambda x: x[2]).min().map(
            lambda x: x[0], output_type=Types.STRING()
        ).key_by(lambda x: x).max_by().add_sink(self.test_sink)
...
class ProcessDataStreamBatchTests(
    DataStreamBatchTests, ProcessDataStreamTests, PyFlinkBatchTestCase
):
    def test_keyed_sum(self):
        self.env.set_parallelism(1)
        ds = self.env.from_collection(
        ...
        ds.key_by(lambda x: x[0]).sum("v2").key_by(lambda x: x[1]).sum().flat_map(
            flat_map_func1, output_type=Types.TUPLE([Types.INT(), Types.INT()])
        ).key_by(lambda x: x[0]).sum(1).flat_map(flat_map_func2, output_type=Types.INT()).key_by(
            lambda x: x
        ).sum().add_sink(self.test_sink)
    ...
    def test_keyed_min_by_and_max(self):
        ...
        ds = self.env.from_collection(
            ...
            type_info=Types.ROW_NAMED(
                ["v1", "v2", "v3"], [Types.INT(), Types.STRING(), Types.INT()]
            ),
        )
        ...
        ).key_by(lambda x: x[2]).max(0).flat_map(
            flat_map_func1, output_type=Types.TUPLE([Types.INT(), Types.INT()])
        ).key_by(lambda x: [1]).min_by().flat_map(flat_map_func2, output_type=Types.INT()).key_by(
            lambda x: x
        ).max_by().add_sink(self.test_sink)
...
class EmbeddedDataStreamStreamTests(DataStreamStreamingTests, PyFlinkStreamingTestCase):
    ...
    def test_metrics(self):
        ds = self.env.from_collection(
            ...
            type_info=Types.TUPLE([Types.STRING(), Types.STRING(), Types.BIG_DEC()]),
        )
...
class CommonDataStreamTests(PyFlinkTestCase):
    ...
    def test_project(self):
        ds = self.env.from_collection(
            [[1, 2, 3, 4], [5, 6, 7, 8]],
            type_info=Types.TUPLE([Types.INT(), Types.INT(), Types.INT(), Types.INT()]),
        )
    ...
    def test_slot_sharing_group(self):
        ...
        ds_1.slot_sharing_group(SlotSharingGroup.builder(slot_sharing_group_1).build()).map(
            lambda x: x + 1
        ).set_parallelism(3).name(map_operator_name).slot_sharing_group(
    ...
    def test_chaining_strategy(self):
        ...
        ds.map(lambda x: x).set_parallelism(2).name(chained_operator_name_0).map(
            lambda x: x
        ).set_parallelism(2).name(chained_operator_name_1).map(lambda x: x).set_parallelism(2).name(
            chained_operator_name_2
        ).add_sink(self.test_sink)

        def assert_chainable(
            j_stream_graph, expected_upstream_chainable, expected_downstream_chainable
        ):
            ...
            for j_stream_node in j_stream_nodes:
                if j_stream_node.getOperatorName() == chained_operator_name_1:
                    ...
                    upstream_chainable = JStreamingJobGraphGenerator.isChainable(
                        j_in_stream_edge, j_stream_graph
                    )
        ...
        ds.map(lambda x: x).set_parallelism(2).name(chained_operator_name_0).map(
            lambda x: x
        ).set_parallelism(2).name(chained_operator_name_1).start_new_chain().map(
            lambda x: x
        ).set_parallelism(2).name(chained_operator_name_2).add_sink(self.test_sink)
        ...
        ds.map(lambda x: x).set_parallelism(2).name(chained_operator_name_0).map(
            lambda x: x
        ).set_parallelism(2).name(chained_operator_name_1).disable_chaining().map(
            lambda x: x
        ).set_parallelism(2).name(chained_operator_name_2).add_sink(self.test_sink)
    ...
    def test_execute_and_collect(self):
        ...
        ds = self.env.from_collection(
            collection=test_data, type_info=Types.TUPLE([Types.STRING(), Types.INT()])
        )
        ...
        test_data = [
            (
                ...
                decimal.Decimal("1000000000000000000.0599999999999" "9999899999999999"),
            ),
            (
                2,
                None,
                ...
                decimal.Decimal("2000000000000000000.061111111111111" "11111111111111"),
            ),
        ]
        expected = test_data
        ds = self.env.from_collection(test_data)
    ...
    def test_data_with_custom_class(self):
        ...
        ds.map(lambda a: Data(a[0], a[1])).flat_map(
            lambda data: [data.name for _ in range(data.num)]
        ).add_sink(self.test_sink)
