...
class SlotSharingGroupTests(PyFlinkTestCase):
    ...
    def test_build_slot_sharing_group_with_illegal_config(self):
        with self.assertRaises(Exception):
            SlotSharingGroup.builder("slot_sharing_group").set_cpu_cores(1.0).set_task_heap_memory(
                MemorySize(bytes_size=0)
            ).set_task_off_heap_memory_mb(10).build()

    def test_build_slot_sharing_group_without_all_required_config(self):
        with self.assertRaises(Exception):
            SlotSharingGroup.builder("slot_sharing_group").set_cpu_cores(
                1.0
            ).set_task_off_heap_memory_mb(10).build()
