...
class EmbeddedRocksDBStateBackendTests(PyFlinkTestCase):
    ...
    def test_get_set_db_storage_paths(self):
        if on_windows():
            ...
            expected = [
                "C:\\var\\db_storage_dir1",
                "C:\\var\\db_storage_dir2",
                "C:\\var\\db_storage_dir3",
            ]
        else:
            storage_path = [
                "file://var/db_storage_dir1/",
    ...
    def test_get_set_predefined_options(self):
        ...
        self.assertEqual(
            state_backend.get_predefined_options(),
        ...
        self.assertEqual(
            state_backend.get_predefined_options(), PredefinedOptions.SPINNING_DISK_OPTIMIZED
        )

        state_backend.set_predefined_options(PredefinedOptions.FLASH_SSD_OPTIMIZED)

        self.assertEqual(
            state_backend.get_predefined_options(), PredefinedOptions.FLASH_SSD_OPTIMIZED
        )
    ...
    def test_get_set_options(self):
        ...
        state_backend.set_options(
            "org.apache.flink.state.rocksdb." "RocksDBStateBackendConfigTest$TestOptionsFactory"
        )

        self.assertEqual(
            state_backend.get_options(),
            "org.apache.flink.state.rocksdb." "RocksDBStateBackendConfigTest$TestOptionsFactory",
        )
...
class CustomStateBackendTests(PyFlinkTestCase):
    def test_create_custom_state_backend(self):
        ...
        j_factory = load_java_class(
            "org.apache.flink.streaming.runtime.tasks."
            "StreamTaskTest$TestMemoryStateBackendFactory"
        ).newInstance()
        context_classloader = gateway.jvm.Thread.currentThread().getContextClassLoader()
        state_backend = _from_j_state_backend(
            j_factory.createFromConfig(j_config, context_classloader)
        )
