...
class StreamExecutionEnvironmentTests(PyFlinkTestCase):
    ...
    def test_execute(self):
        ...
        execution_result = exec_insert_table(
            t_env.from_elements([(1, "Hi", "Hello")], ["a", "b", "c"]), "Results"
        )
    ...
    def test_add_python_file(self):
        ...
        get_j_env_configuration(env._j_stream_execution_environment).setString(
            "taskmanager.numberOfTaskSlots", "10"
        )
        env.add_python_file(python_file_path)
        ds = env.from_collection([1, 2, 3, 4, 5])
        ds = (
            ds.map(plus_two_map, Types.LONG())
        ...
        t_env.to_append_stream(tab, Types.ROW([Types.LONG()])).map(lambda i: i[0]).add_sink(
            self.test_sink
        )
    ...
    def test_generate_stream_graph_with_dependencies(self):
        ...
        python_dependency_config = dict(
            get_gateway()
            .jvm.org.apache.flink.python.util.PythonDependencyUtils.configurePythonDependencies(
                env._j_stream_execution_environment.getConfiguration()
            )
            .toMap()
        )
    ...
    def test_register_slot_sharing_group(self):
        slot_sharing_group_1 = (
            SlotSharingGroup.builder("slot_sharing_group_1")
            .set_cpu_cores(1.0)
            .set_task_heap_memory_mb(100)
            .build()
        )
        slot_sharing_group_2 = (
            SlotSharingGroup.builder("slot_sharing_group_2")
            .set_cpu_cores(2.0)
            .set_task_heap_memory_mb(200)
            .build()
        )
        ...
        ds.map(lambda x: x + 1).set_parallelism(3).slot_sharing_group(
            "slot_sharing_group_2"
