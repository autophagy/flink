...
class WindowTests(object):
    ...
    def test_event_time_tumbling_window(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(TumblingEventTimeWindows.of(Time.milliseconds(5))).process(
    ...
    def test_count_tumbling_window(self):
        ...
        data_stream.key_by(lambda x: x[1], key_type=Types.STRING()).count_window(3).apply(
    ...
    def test_event_time_sliding_window(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(SlidingEventTimeWindows.of(Time.milliseconds(5), Time.milliseconds(2))).process(
            CountWindowProcessFunction(),
            Types.TUPLE([Types.STRING(), Types.LONG(), Types.LONG(), Types.INT()]),
        ).add_sink(self.test_sink)
    ...
    def test_count_sliding_window(self):
        ...
        data_stream.key_by(lambda x: x[1], key_type=Types.STRING()).window(
            CountSlidingWindowAssigner(2, 1)
        ).apply(SumWindowFunction(), Types.TUPLE([Types.STRING(), Types.INT()])).add_sink(
            self.test_sink
        )
    ...
    def test_event_time_session_window(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(EventTimeSessionWindows.with_gap(Time.milliseconds(5))).process(
            CountWindowProcessFunction(),
            Types.TUPLE([Types.STRING(), Types.LONG(), Types.LONG(), Types.INT()]),
        ).add_sink(self.test_sink)
    ...
    def test_window_reduce_passthrough(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(EventTimeSessionWindows.with_gap(Time.milliseconds(2))).reduce(
            lambda a, b: (b[0], a[1] + b[1]), output_type=Types.TUPLE([Types.STRING(), Types.INT()])
        ).add_sink(self.test_sink)
    ...
    def test_window_aggregate_passthrough(self):
        ...
        class MyAggregateFunction(AggregateFunction):
            ...
            def add(
                self, value: Tuple[str, int], accumulator: Tuple[str, Dict[int, int]]
            ...
            def merge(
                self, acc_a: Tuple[str, Dict[int, int]], acc_b: Tuple[str, Dict[int, int]]
            ) -> Tuple[str, Dict[int, int]]:
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(EventTimeSessionWindows.with_gap(Time.milliseconds(2))).aggregate(
            MyAggregateFunction(), output_type=Types.TUPLE([Types.STRING(), Types.INT()])
        ).add_sink(self.test_sink)
    ...
    def test_session_window_late_merge(self):
        data_stream: DataStream = self.env.from_collection(
            [("hi", 0), ("hi", 8), ("hi", 4)], type_info=Types.TUPLE([Types.STRING(), Types.INT()])
        )
        watermark_strategy = WatermarkStrategy.for_monotonous_timestamps().with_timestamp_assigner(
            SecondColumnTimestampAssigner()
        )
    ...
    def test_event_time_session_window_with_purging_trigger(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(EventTimeSessionWindows.with_gap(Time.milliseconds(3))).trigger(
    ...
    def test_global_window_with_purging_trigger(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(
            lambda x: x[0], key_type=Types.STRING()
        ).window(GlobalWindows.create()).trigger(PurgingTrigger.of(CountTrigger.of(2))).process(
            MyProcessFunction(), Types.TUPLE([Types.STRING(), Types.INT()])
        ).add_sink(self.test_sink)
    ...
    def test_event_time_tumbling_window_all(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).window_all(
            TumblingEventTimeWindows.of(Time.milliseconds(5))
        ).process(
            CountAllWindowProcessFunction(), Types.TUPLE([Types.LONG(), Types.LONG(), Types.INT()])
        ).add_sink(self.test_sink)
    ...
    def test_window_all_reduce(self):
        ...
        data_stream.assign_timestamps_and_watermarks(watermark_strategy).window_all(
            EventTimeSessionWindows.with_gap(Time.milliseconds(2))
        ).reduce(
            lambda a, b: (a[0], a[1] + b[1]), output_type=Types.TUPLE([Types.STRING(), Types.INT()])
        ).add_sink(self.test_sink)
    ...
    def test_window_all_aggregate_process(self):
        ...
        class MyProcessWindowFunction(ProcessAllWindowFunction):
            def process(
                self, context: ProcessAllWindowFunction.Context, elements: Iterable[Tuple[str, int]]
            ) -> Iterable[str]:
                agg_result = next(iter(elements))
                yield "key {} timestamp sum {}".format(agg_result[0], agg_result[1])
        ...
        results = self.test_sink.get_results()
        expected = ["key b timestamp sum 6", "key b timestamp sum 23", "key a timestamp sum 15"]
        self.assert_equals_sorted(expected, results)
    ...
    def test_side_output_late_data(self):
        ...
        watermark_strategy = WatermarkStrategy(
            jvm.org.apache.flink.api.common.eventtime.WatermarkStrategy.forGenerator(
                jvm.org.apache.flink.streaming.api.functions.python.eventtime.PerElementWatermarkGenerator.getSupplier()
            )
        ...
        ds2 = (
            ds1.assign_timestamps_and_watermarks(watermark_strategy)
            .key_by(lambda e: e[0])
...
class EmbeddedWindowTests(WindowTests, PyFlinkStreamingTestCase):
    ...
    def test_chained_window(self):
        ...
        ds = self.env.from_collection(
            [
                (1676461680000, "a1", "b1", 1),
                (1676461680000, "a1", "b1", 1),
                (1676461680000, "a2", "b2", 1),
                (1676461680000, "a1", "b2", 1),
                (1676461740000, "a1", "b1", 1),
                (1676461740000, "a2", "b2", 1),
            ]
        ...
        ds.key_by(lambda x: (x[0], x[1], x[2])).window(
