...
################################################################################
from pyflink.fn_execution.embedded.operations import (
    OneInputFunctionOperation,
...
def create_scalar_operation_from_proto(
    proto,
    input_coder_info,
    output_coder_into,
    ...
    input_data_converter = from_schema_proto(
        parse_coder_proto(input_coder_info).flatten_row_type.schema, one_arg_optimization
    )

    output_data_converter = from_schema_proto(
        parse_coder_proto(output_coder_into).flatten_row_type.schema, one_result_optimization
    )
...
def create_one_input_user_defined_data_stream_function_from_protos(
    ...
    serialized_fns = [
        pare_user_defined_data_stream_function_proto(proto) for proto in function_infos
    ]
    input_data_converter = from_type_info_proto(
        parse_coder_proto(input_coder_info).raw_type.type_info
    )
    output_data_converter = from_type_info_proto(
        parse_coder_proto(output_coder_info).raw_type.type_info
    )
...
def create_two_input_user_defined_data_stream_function_from_protos(
    ...
    input_data_converter1 = from_type_info_proto(
        parse_coder_proto(input_coder_info1).raw_type.type_info
    )

    input_data_converter2 = from_type_info_proto(
        parse_coder_proto(input_coder_info2).raw_type.type_info
    )
