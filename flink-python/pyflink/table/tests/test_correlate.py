...
class CorrelateTests(PyFlinkUTTestCase):
    def test_join_lateral(self):
        ...
        t_env.create_java_temporary_system_function(
            "split", "org.apache.flink.table.utils.TestingFunctions$TableFunc1"
        )
    ...
    def test_join_lateral_with_join_predicate(self):
        ...
        result = source.join_lateral(
            expr.call("split", source.words).alias("word"), expr.col("id") == expr.col("word")
        )
    ...
    def test_left_outer_join_lateral_with_join_predicate(self):
        ...
        result = source.left_outer_join_lateral(
            expr.call("split", source.words).alias("word"), expr.lit(True)
        )
