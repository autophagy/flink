...
class StreamTableExplainTests(PyFlinkStreamTableTestCase):
    def test_explain(self):
        ...
        result = (
            t.group_by(t.c)
            .select(t.a.sum, t.c.alias("b"))
            .explain(ExplainDetail.CHANGELOG_MODE, ExplainDetail.PLAN_ADVICE)
        )
        assert isinstance(result, str)
        self.assertGreaterEqual(result.find("== Optimized Physical Plan With Advice =="), 0)
        self.assertGreaterEqual(
            result.find(
                "advice[1]: [ADVICE] You might want to enable "
