...
from pyflink.table.expression import (
    TimeIntervalUnit,
    TimePointUnit,
    JsonExistsOnError,
...
class PyFlinkBatchExpressionTests(PyFlinkTestCase):
    def test_expression(self):
        ...
        self.assertEqual("PERCENTILE(a, 0.5, b)", str(expr1.percentile(0.5, expr2)))
        self.assertEqual("PERCENTILE(a, array(0.1, 0.5))", str(expr1.percentile(array(0.1, 0.5))))
        self.assertEqual(
            "PERCENTILE(a, array(0.1, 0.5), b)", str(expr1.percentile(array(0.1, 0.5), expr2))
        )
        ...
        self.assertEqual(
            "JSON_QUERY('{}', '$.x', STRING, WITHOUT_ARRAY, NULL, EMPTY_ARRAY)",
            str(
                lit("{}").json_query(
                    "$.x",
    ...
    def test_expressions(self):
        ...
        self.assertEqual(
            "toDate('2018-03-18', 'yyyy-MM-dd')", str(to_date("2018-03-18", "yyyy-MM-dd"))
        )
        self.assertEqual("TO_TIMESTAMP_LTZ(100)", str(to_timestamp_ltz(100)))
        self.assertEqual(
            "TO_TIMESTAMP_LTZ('2023-01-01 00:00:00')", str(to_timestamp_ltz("2023-01-01 00:00:00"))
        )
        self.assertEqual(
            "TO_TIMESTAMP_LTZ('01/01/2023 00:00:00', 'MM/dd/yyyy HH:mm:ss')",
        ...
        self.assertEqual(
            "toTimestamp('1970-01-01 08:01:40')", str(to_timestamp("1970-01-01 08:01:40"))
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            str(
                temporal_overlaps(
                    lit("2:55:00").to_time, lit(1).hours, lit("3:30:00").to_time, lit(2).hours
                )
            ),
        )
        self.assertEqual(
            "dateFormat(time, '%Y, %d %M')", str(date_format(col("time"), "%Y, %d %M"))
        )
        self.assertEqual(
            "timestampDiff(DAY, cast('2016-06-15', DATE), cast('2016-06-18', DATE))",
            str(
                timestamp_diff(
                    TimePointUnit.DAY, lit("2016-06-15").to_date, lit("2016-06-18").to_date
                )
            ),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "fromUnixtime(1, 'yy-MM-dd HH-mm-ss')", str(from_unixtime(1, "yy-MM-dd HH-mm-ss"))
        )
        self.assertEqual("unixTimestamp()", str(unix_timestamp()))
        self.assertEqual(
            "unixTimestamp('2015-07-24 10:00:00')", str(unix_timestamp("2015-07-24 10:00:00"))
        )
        self.assertEqual(
            "unixTimestamp('2015-07-24 10:00:00', 'yy-MM-dd HH-mm-ss')",
        ...
        self.assertEqual(
            "map('key1', 1, 'key2', 2, 'key3', 3)", str(map_("key1", 1, "key2", 2, "key3", 3))
        )
