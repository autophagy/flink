...
class BatchPandasUDAFITTests(PyFlinkBatchTableTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.t_env.create_temporary_system_function(
            "max_add", udaf(MaxAdd(), result_type=DataTypes.INT(), func_type="pandas")
        )
    ...
    def test_check_result_type(self):
        ...
        with self.assertRaises(
            ...
            udaf(
                pandas_udaf,
                result_type=DataTypes.MAP(DataTypes.INT(), DataTypes.INT()),
                func_type="pandas",
            )
    ...
    def test_group_aggregate_function(self):
        ...
        t.group_by(t.a).select(
            t.a, mean_udaf(add(t.b)), max_udaf(substract(t.c)), multiply_udaf(t.b, "abc")
    ...
    def test_group_aggregate_without_keys(self):
        ...
        self.t_env.execute_sql(sink_table_ddl)
        min_add = udaf(
            lambda a, b, c: a.min() + b.min() + c.min(),
    ...
    def test_group_aggregate_with_aux_group(self):
        ...
        t.group_by(t.a).select(t.a, (t.a + 1).alias("b"), (t.a + 2).alias("c")).group_by(
            t.a, t.b
        ).select(t.a, t.b, mean_udaf(t.b), call("max_add", t.b, t.c, 1)).execute_insert(
            sink_table
        ).wait()
        actual = source_sink_utils.results()
    ...
    def test_tumble_group_window_aggregate_function(self):
        ...
        t.window(tumble_window).group_by(col("w")).select(
            col("w").start, col("w").end, mean_udaf(t.b)
        ).execute_insert(sink_table).wait()
    ...
    def test_slide_group_window_aggregate_function(self):
        ...
        t.window(slide_window).group_by(t.a, col("w")).select(
            t.a, col("w").start, col("w").end, mean_udaf(t.b), call("max_add", t.b, t.c, 1)
        ).execute_insert(sink_table).wait()
        actual = source_sink_utils.results()
...
class StreamPandasUDAFITTests(PyFlinkStreamTableTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.t_env.create_temporary_system_function("mean_udaf", mean_udaf)
        max_add_min_udaf = udaf(
            lambda a: a.max() + a.min(), result_type="SMALLINT", func_type="pandas"
        )
    ...
    def test_sliding_group_window_over_time(self):
        ...
        self.t_env.execute_sql(sink_table_ddl)
        t.window(
            Slide.over(lit(1).hours).every(lit(30).minutes).on(col("rowtime")).alias("w")
        ).group_by(t.a, t.b, col("w")).select(
            t.a, col("w").start, col("w").end, mean_udaf(t.c).alias("b")
        ).execute_insert(sink_table).wait()
        actual = source_sink_utils.results()
        self.assert_equals(
            actual,
    ...
    def test_sliding_group_window_over_count(self):
        ...
        self.t_env.execute_sql(sink_table_ddl)
        t.window(
            Slide.over(row_interval(2)).every(row_interval(1)).on(t.protime).alias("w")
        ).group_by(t.a, t.b, col("w")).select(t.a, mean_udaf(t.c).alias("b")).execute_insert(
            sink_table
        ).wait()
        actual = source_sink_utils.results()
    ...
    def test_tumbling_group_window_over_time(self):
        ...
        """
        self.t_env.execute_sql(sink_table_ddl)
        t.window(Tumble.over(lit(1).hours).on(t.rowtime).alias("w")).group_by(
            t.a, t.b, col("w")
        ).select(
            t.a, col("w").start, col("w").end, col("w").rowtime, mean_udaf(t.c).alias("b")
        ).execute_insert(sink_table).wait()
        actual = source_sink_utils.results()
        self.assert_equals(
            actual,
    ...
    def test_tumbling_group_window_over_count(self):
        ...
        """
        self.t_env.execute_sql(sink_table_ddl)
        t.window(Tumble.over(row_interval(2)).on(t.protime).alias("w")).group_by(
            t.a, t.b, col("w")
        ).select(t.a, mean_udaf(t.c).alias("b")).execute_insert(sink_table).wait()
        actual = source_sink_utils.results()
