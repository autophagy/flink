...
class PandasUDFITTests(object):
    def test_basic_functionality(self):
        ...
        t.where(add_one(t.b) <= 3).select(
            t.a, t.b + 1, add(t.a + 1, subtract_one(t.c)) + 2, add(add_one(t.a), 1)
        ).execute_insert("Results_test_basic_functionality").wait()
    ...
    def test_all_data_types(self):
        ...
        def tinyint_func(tinyint_param):
            ...
            assert isinstance(tinyint_param[0], np.int8), "tinyint_param of wrong type %s !" % type(
                tinyint_param[0]
            )
        ...
        def int_func(int_param):
            ...
            assert isinstance(int_param[0], np.int32), "int_param of wrong type %s !" % type(
                int_param[0]
            )
        ...
        def bigint_func(bigint_param):
            ...
            assert isinstance(bigint_param[0], np.int64), "bigint_param of wrong type %s !" % type(
                bigint_param[0]
            )
            return bigint_param
        ...
        def float_func(float_param):
            ...
            assert isinstance(float_param[0], np.float32), "float_param of wrong type %s !" % type(
                float_param[0]
            )
            return float_param
        ...
        def varchar_func(varchar_param):
            ...
            assert isinstance(varchar_param[0], str), "varchar_param of wrong type %s !" % type(
                varchar_param[0]
            )
        ...
        def date_func(date_param):
            ...
            assert isinstance(date_param[0], datetime.date), "date_param of wrong type %s !" % type(
                date_param[0]
            )
            return date_param
        ...
        def time_func(time_param):
            ...
            assert isinstance(time_param[0], datetime.time), "time_param of wrong type %s !" % type(
                time_param[0]
            )
            return time_param
        ...
        def timestamp_func(timestamp_param):
            ...
            assert timestamp_param[0] == timestamp_value, (
                "timestamp_param is wrong value %s, should be %s!"
        ...
        def array_func(array_param):
            ...
            assert isinstance(array_param[0], np.ndarray), "array_param of wrong type %s !" % type(
                array_param[0]
            )
            return array_param

        array_str_func = udf(
            array_func, result_type=DataTypes.ARRAY(DataTypes.STRING()), func_type="pandas"
        )

        array_timestamp_func = udf(
            array_func, result_type=DataTypes.ARRAY(DataTypes.TIMESTAMP(3)), func_type="pandas"
        )

        array_int_func = udf(
            array_func, result_type=DataTypes.ARRAY(DataTypes.INT()), func_type="pandas"
        )
        ...
        def row_func(row_param):
            ...
            assert isinstance(row_param.f1[0], np.int32), "row_param.f1 of wrong type %s !" % type(
                row_param.f1[0]
            )
            assert isinstance(row_param.f2, pd.Series)
            assert isinstance(row_param.f2[0], str), "row_param.f2 of wrong type %s !" % type(
                row_param.f2[0]
            )
        ...
        def binary_func(binary_param):
            ...
            assert isinstance(binary_param[0], bytes), "binary_param of wrong type %s !" % type(
                binary_param[0]
            )
    ...
    def test_invalid_pandas_udf(self):
        ...
        msg = (
            "The result length '0' of Pandas UDF 'length_mismatch' is not equal "
        ...
        msg = (
            "The result type of Pandas UDF 'result_type_not_series' must be pandas.Series or "
    ...
    def test_data_types(self):
        ...
        def local_zoned_timestamp_func(local_zoned_timestamp_param):
            ...
            assert isinstance(local_zoned_timestamp_param[0], datetime.datetime), (
                "local_zoned_timestamp_param of wrong type %s !"
                % type(local_zoned_timestamp_param[0])
            )
        ...
        t.select(local_zoned_timestamp_func(local_zoned_timestamp_func(t.a))).execute_insert(
            "Results_test_data_types"
        ).wait()
        actual = source_sink_utils.results()
