...
class RowBasedOperationTests(object):
    def test_map(self):
        ...
        func2 = udf(
            lambda x: Row(x.a + 1, x.b * 2),
            result_type=DataTypes.ROW(
                [DataTypes.FIELD("a", DataTypes.BIGINT()), DataTypes.FIELD("b", DataTypes.BIGINT())]
            ),
        )

        t.map(func(t.b)).alias("a", "b").map(func(t.a)).map(func2).execute_insert(
            "Results_test_map"
        ).wait()
    ...
    def test_map_with_pandas_udf(self):
        ...
        pandas_udf = udf(
            func,
            result_type=DataTypes.ROW(
                [DataTypes.FIELD("c", DataTypes.BIGINT()), DataTypes.FIELD("d", DataTypes.BIGINT())]
            ),
            func_type="pandas",
        )
        ...
        t.map(pandas_udf).map(pandas_udf_2).map(general_udf).execute_insert(
            "Results_test_map_with_pandas_udf"
        ).wait()
        actual = source_sink_utils.results()
    ...
    def test_flat_map(self):
        ...
        t.flat_map(split).alias("a", "b").flat_map(split).alias("a", "b").join_lateral(
            split.alias("c", "d")
        ).left_outer_join_lateral(split.alias("e", "f")).execute_insert(
...
class BatchRowBasedOperationITTests(RowBasedOperationTests, PyFlinkBatchTableTestCase):
    def test_aggregate_with_pandas_udaf(self):
        ...
        self.t_env.execute_sql(sink_table_ddl)
        pandas_udaf = udaf(
            lambda pd: (pd.b.mean(), pd.a.max()),
            result_type=DataTypes.ROW(
                [DataTypes.FIELD("a", DataTypes.FLOAT()), DataTypes.FIELD("b", DataTypes.INT())]
            ),
            func_type="pandas",
        )
        t.select(t.a, t.b).group_by(t.a).aggregate(pandas_udaf).select(
            expr.col("*")
        ).execute_insert("Results_test_aggregate_with_pandas_udaf").wait()
        actual = source_sink_utils.results()
    ...
    def test_aggregate_with_pandas_udaf_without_keys(self):
        ...
        self.t_env.execute_sql(sink_table_ddl)
        pandas_udaf = udaf(
            lambda pd: Row(pd.b.mean(), pd.b.max()),
            result_type=DataTypes.ROW(
                [DataTypes.FIELD("a", DataTypes.FLOAT()), DataTypes.FIELD("b", DataTypes.INT())]
            ),
            func_type="pandas",
        )
        t.select(t.b).aggregate(pandas_udaf.alias("a", "b")).select(t.a, t.b).execute_insert(
            "Results_test_aggregate_with_pandas_udaf_without_keys"
        ).wait()
        actual = source_sink_utils.results()
    ...
    def test_window_aggregate_with_pandas_udaf(self):
        ...
        t.select(t.b, t.rowtime).window(tumble_window).group_by(expr.col("w")).aggregate(
            pandas_udaf.alias("d", "e")
        ).select(expr.col("w").rowtime, expr.col("d"), expr.col("e")).execute_insert(
            "Results_test_window_aggregate_with_pandas_udaf"
        ).wait()

        actual = source_sink_utils.results()
        self.assert_equals(
            actual, ["+I[2018-03-11 03:59:59.999, 2.2, 3]", "+I[2018-03-11 04:59:59.999, 8.0, 8]"]
        )
