...
class TableEnvironmentTest(PyFlinkUTTestCase):
    def test_set_sys_executable_for_local_mode(self):
        jvm = get_gateway().jvm
        actual_executable = get_j_env_configuration(self.t_env._get_j_env()).getString(
            jvm.PythonOptions.PYTHON_EXECUTABLE.key(), None
        )
    ...
    def test_load_module_twice(self):
        ...
        self.assertRaisesRegex(
            ...
            t_env.load_module,
            "core",
            Module(get_gateway().jvm.org.apache.flink.table.module.CoreModule.INSTANCE),
        )
    ...
    def test_unload_module_twice(self):
        ...
        self.assertRaisesRegex(
            Py4JJavaError, "No module with name 'core' exists", t_env.unload_module, "core"
        )
    ...
    def test_use_modules(self):
        ...
        t_env = self.t_env
        t_env.load_module(
            "x", Module(get_gateway().jvm.org.apache.flink.table.utils.ModuleMock("x"))
        )
        t_env.load_module(
            "y", Module(get_gateway().jvm.org.apache.flink.table.utils.ModuleMock("y"))
        )
    ...
    def check_list_full_modules(self, used_module_cnt: int, *expected_loaded_modules: str):
        self.assert_equals(
            self.t_env.list_full_modules(),
            [
                ModuleEntry(module, expected_loaded_modules.index(module) < used_module_cnt)
    ...
    def test_unload_and_load_module(self):
        ...
        t_env.unload_module("core")
        t_env.load_module(
            "core", Module(get_gateway().jvm.org.apache.flink.table.module.CoreModule.INSTANCE)
        )
    ...
    def test_create_and_drop_java_function(self):
        ...
        self.assert_equals(
            t_env.list_user_defined_functions(), ["scalar_func", "agg_func", "table_func"]
        )
    ...
    def test_create_temporary_table_if_not_exists_from_descriptor(self):
        ...
        self.t_env.create_temporary_table(
            "T",
            TableDescriptor.for_connector("fake").schema(schema).option("a", "Test").build(),
            True,
        )
        ...
        self.assertIn(
            "Temporary table '`default_catalog`.`default_database`.`T`' already exists", error_msg
        )
    ...
    def test_table_from_descriptor(self):
        ...
        self.assertEqual(
            schema, Schema.new_builder().from_resolved_schema(table.get_resolved_schema()).build()
        )
    ...
    def test_udt(self):
        self.t_env.from_elements(
            [
                (DenseVector([1, 2, 3, 4]), 0.0, 1.0),
                (DenseVector([2, 2, 3, 4]), 0.0, 2.0),
                (DenseVector([3, 2, 3, 4]), 0.0, 3.0),
                (DenseVector([4, 2, 3, 4]), 0.0, 4.0),
                (DenseVector([5, 2, 3, 4]), 0.0, 5.0),
                (DenseVector([11, 2, 3, 4]), 1.0, 1.0),
                (DenseVector([12, 2, 3, 4]), 1.0, 2.0),
                (DenseVector([13, 2, 3, 4]), 1.0, 3.0),
                (DenseVector([14, 2, 3, 4]), 1.0, 4.0),
                (DenseVector([15, 2, 3, 4]), 1.0, 5.0),
            ],
            DataTypes.ROW(
                [
    ...
    def assert_created_table_from_descriptor(self, schema: Schema):
        ...
        self.t_env.create_table(
            "T", TableDescriptor.for_connector("fake").schema(schema).option("a", "Test").build()
        )
...
class DataStreamConversionTestCases(PyFlinkUTTestCase):
  ...
  `f0` RAW('[B', '...')
        ...
        with result.collect() as result:
            ...
            expected_result = [
                item for item in map(str, [Row((1,)), Row((2,)), Row((3,)), Row((4,)), Row((5,))])
            ]
    ...
    def test_to_data_stream_local_time(self):
        ...
        results.sort()
        expected = [
            "+I[1, 1970-01-02, 03:04:05, 1970-01-02T03:04:05]",
    ...
    def test_from_data_stream(self):
        ...
        ds = ds.map(lambda x: x, Types.ROW([Types.INT(), Types.STRING(), Types.STRING()])).map(
            lambda x: x, Types.ROW([Types.INT(), Types.STRING(), Types.STRING()])
        )
    ...
    def test_from_data_stream_with_schema(self):
        ds = self.env.from_collection(
            ...
            type_info=Types.ROW_NAMED(
                ["a", "b", "c"], [Types.INT(), Types.STRING(), Types.STRING()]
            ),
        )
        ...
        with result.collect() as result:
            collected_result = [str(item) for item in result]
            expected_result = [
                item for item in map(str, [Row(1, "Hi", "Hello"), Row(2, "Hello", "Hi")])
            ]
            expected_result.sort()
            collected_result.sort()
            self.assertEqual(expected_result, collected_result)
  ...
  WATERMARK FOR `rowtime`: TIMESTAMP_LTZ(3) AS SOURCE_WATERMARK()
        ...
        result = self.t_env.execute_sql(
            "SELECT " "c, SUM(b) " "FROM t " "GROUP BY c, TUMBLE(rowtime, INTERVAL '0.005' SECOND)"
        )
        with result.collect() as result:
            collected_result = [str(item) for item in result]
            expected_result = [
                item for item in map(str, [Row("a", 47), Row("c", 1000), Row("c", 1000)])
            ]
            expected_result.sort()
            collected_result.sort()
            self.assertEqual(expected_result, collected_result)
        ...
        ds = self.t_env.to_data_stream(table)
        ds.key_by(lambda k: k.c, key_type=Types.STRING()).window(MyTumblingEventTimeWindow()).apply(
            SumWindowFunction(), Types.TUPLE([Types.STRING(), Types.INT()])
    ...
    def test_from_and_to_changelog_stream_event_time(self):
        ...
        changelog_stream = ds.map(
            lambda t: Row(t.f1, t.f2), Types.ROW([Types.INT(), Types.STRING()])
        )
        ...
        result.key_by(lambda k: k.f1).window(MyTumblingEventTimeWindow()).apply(
            SumWindowFunction(), Types.TUPLE([Types.STRING(), Types.INT()])
        ).add_sink(self.test_sink)
        self.env.execute()
        expected_results = ["(A,47)", "(C,1000)", "(C,1000)"]
    ...
    def test_to_append_stream(self):
        ...
        t_env = StreamTableEnvironment.create(
            self.env, environment_settings=EnvironmentSettings.in_streaming_mode()
        )
        table = t_env.from_elements([(1, "Hi", "Hello"), (2, "Hello", "Hi")], ["a", "b", "c"])
        new_table = table.select(table.a + 1, table.b + "flink", table.c)
        ds = t_env.to_append_stream(
            table=new_table, type_info=Types.ROW([Types.LONG(), Types.STRING(), Types.STRING()])
        )
    ...
    def test_to_retract_stream(self):
        ...
        ds = t_env.to_retract_stream(
            table=new_table, type_info=Types.ROW([Types.LONG(), Types.STRING()])
        )
        test_sink = DataStreamTestSinkFunction()
        ...
        expected = [
            "(True, Row(f0=1, f1='Hello'))",
            "(False, Row(f0=1, f1='Hello'))",
...
class StreamTableEnvironmentTests(PyFlinkStreamTableTestCase):
    def test_collect_with_retract(self):
        ...
        with table_result.collect() as result:
            ...
            collected_result = [
                str(result) + "," + str(result.get_row_kind()) for result in collected_result
            ]
            expected_result = [
                Row(1, "a"),
                Row(1, "a"),
    ...
    def test_row_form_consistency_with_elements(self):
        ...
        valid_tuple_elements = [([("pyflink", True), ("pyflink", False), ("pyflink", True)],)]
        valid_list_elements = [([["pyflink", True], ["pyflink", False], ["pyflink", True]],)]
        valid_keyword_row = [
        ...
        expected_valid_result = [
            Row([Row("pyflink", True), Row("pyflink", False), Row("pyflink", True)])
        ]
        ...
        invalid_tuple_elements = [([("pyflink", True), ("pyflink", False), (True, "pyflink")],)]
        invalid_list_elements = [([["pyflink", True], ["pyflink", False], [True, "pyflink"]],)]
        invalid_keyword_row = [
...
class MyTumblingEventTimeWindow(MergingWindowAssigner[tuple, TimeWindow]):
    def merge_windows(
        self, windows, callback: "MergingWindowAssigner.MergeCallback[TimeWindow]"
    ) -> None:
...
class SimpleTimeWindowTrigger(Trigger[tuple, TimeWindow]):
    def on_element(
        self, element: tuple, timestamp: int, window: TimeWindow, ctx: "Trigger.TriggerContext"
    ) -> TriggerResult:
