...
class CountDistinctAggregateFunction(AggregateFunction):
    ...
    def accumulate(self, accumulator, *args):
        input_str = args[0]
        if (
            accumulator[0].is_empty()
            or input_str not in accumulator[0]
...
class StreamTableAggregateTests(PyFlinkStreamTableTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.t_env.create_temporary_system_function(
            "my_count_distinct", CountDistinctAggregateFunction()
        )
    ...
    def test_double_aggregate(self):
        ...
        result = (
            t.group_by(t.c)
            .select(call("my_count", t.a).alias("a"), call("my_sum", t.a).alias("b"), t.c)
            .select(
                call("my_count", col("a")).alias("a"),
    ...
    def test_mixed_with_built_in_functions_without_retract(self):
        self.t_env.get_config().set("parallelism.default", "1")
        t = self.t_env.from_elements(
            [("Hi", 2), ("Hi", 4), (None, None), ("hello2", 8), ("hello", 10)], ["b", "c"]
        )
        self.t_env.create_temporary_view(
    ...
    def test_list_view(self):
        ...
        assert_frame_equal(
            ...
            pd.DataFrame(
                [["Hi,Hi,Hi2,Hi2,Hi3", "Hello"], ["Hi,Hi2,Hi,Hi3,Hi3", "hi"]], columns=["a", "c"]
            ),
        )
    ...
    def test_distinct_and_filter(self):
        ...
        assert_frame_equal(
            result.to_pandas().sort_values(by="a").reset_index(drop=True),
            pd.DataFrame(
                [
                    ["Hi.Hi2.Hi3", "Hi,Hi2,Hi3", "", "hi"],
    ...
    def test_tumbling_group_window_over_time(self):
        ...
        t.window(Tumble.over(lit(1).hours).on(t.rowtime).alias("w")).group_by(t.a, col("w")).select(
            t.a,
            col("w").start,
            col("w").end,
            t.c.count.alias("c"),
    ...
    def test_tumbling_group_window_over_count(self):
        ...
        t.window(Tumble.over(row_interval(2)).on(t.protime).alias("w")).group_by(
            t.a, col("w")
        ).select(t.a, call("my_sum", t.c).alias("b")).execute_insert(sink_table).wait()
        actual = source_sink_utils.results()
    ...
    def test_sliding_group_window_over_time(self):
        ...
        """
        self.t_env.execute_sql(sink_table_ddl)
        t.window(Slide.over(lit(1).hours).every(lit(30).minutes).on(t.rowtime).alias("w")).group_by(
            t.a, col("w")
        ).select(t.a, col("w").start, col("w").end, call("my_sum", t.c).alias("c")).execute_insert(
            sink_table
        ).wait()
        actual = source_sink_utils.results()
        self.assert_equals(
    ...
    def test_sliding_group_window_over_count(self):
        ...
        ).group_by(t.a, col("w")).select(t.a, call("my_sum", t.c).alias("b")).execute_insert(
            sink_table
        ).wait()
        actual = source_sink_utils.results()
    ...
    def test_session_group_window_over_time(self):
        ...
        """
        self.t_env.execute_sql(sink_table_ddl)
        t.window(Session.with_gap(lit(30).minutes).on(t.rowtime).alias("w")).group_by(
            t.a, t.b, col("w")
        ).select(
            t.a, col("w").start, col("w").end, call("my_count", t.c).alias("c")
        ).execute_insert(sink_table).wait()
        actual = source_sink_utils.results()
        self.assert_equals(
            actual,
