...
if sys.version_info < (3, 9):
    print("Python versions prior to 3.9 are not supported for PyFlink.", file=sys.stderr)
    sys.exit(-1)
...
try:
    if in_flink_source:
        ...
        copy(
            os.path.join(this_directory, "pyflink", "bin", PYFLINK_UDF_RUNNER_SH),
            os.path.join(SCRIPTS_TEMP_PATH, PYFLINK_UDF_RUNNER_SH),
        )
        copy(
            os.path.join(this_directory, "pyflink", "bin", PYFLINK_UDF_RUNNER_BAT),
        ...
        with open(os.path.join(LOG_TEMP_PATH, "empty.txt"), "w") as f:
            f.write(
                "This file is used to force setuptools to include the log directory. "
