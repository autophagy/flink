...
/**
 * {@code JobCheckpointCleanup} provides cleanup methods for jobs based on their {@link JobStatus}.
 *
 * @see JobCleanup
 */
// TODO: no tests provided, yet
public interface JobStatusBasedJobCleanup {
     ...
     */
    CompletableFuture<Boolean> cleanupJobData(JobID jobId, JobStatus jobStatus) throws Exception;

    /**
     * Calls {@link #cleanupJobData(JobID, JobStatus)} in a retryable fashion.
     *
    ...
    default CompletableFuture<Boolean> cleanupJobDataWithRetry(
        ...
        return FutureUtils.retryOperation(
                () -> {
                    try {
                        return cleanupJobData(jobId, jobStatus);
                    } catch (Exception e) {
