...
public class ProcessTableFunctionTestHarness<OUT> implements AutoCloseable {
    ...
    static TestHarnessTableSemantics buildTableSemantics(
        ...
        List<int[]> upsertKeyIndices = Collections.emptyList();
        if (tableArg.upsertKey != null) {
            int[] indices = resolveColumnNamesToIndices(tableArg, tableArg.upsertKey, "Upsert key");
            upsertKeyIndices = Arrays.asList(indices);
        }
    ...
    private void invokeEval(TableArgumentInfo activeTableArg, Row activeRow) throws Exception {
        validateInputRowKind(activeTableArg, activeRow.getKind());

        activeRow = stripNonKeyFieldsForKeyOnlyDelete(activeTableArg, activeRow);
    ...
    private Row stripNonKeyFieldsForKeyOnlyDelete(TableArgumentInfo tableArg, Row row) {
        ...
        String[] upsertKeyColumns = tableArg.upsertKey;
        if (upsertKeyColumns == null) {
            return row;
        }

        int[] keyIndicesArray =
                resolveColumnNamesToIndices(tableArg, upsertKeyColumns, "Upsert key");
        Set<Integer> keyIndices = new HashSet<>();
        for (int index : keyIndicesArray) {
    ...
    public static final class TableArgument {
        ...
        @Nullable private final String[] upsertKey;
        ...
        private TableArgument(Builder builder) {
            ...
            this.upsertKey = builder.upsertKey;
        }
        ...
        public static final class Builder {
            ...
            @Nullable private String[] upsertKey;
            ...
            /**
             * Configures the upsert key columns for this table argument.
             *
             * <p>Required when using upsert input modes ({@link ChangelogMode#upsert(boolean)})
             * with a {@code SET_SEMANTIC_TABLE} argument whose partition columns match an input's
             * upsert key.
             *
             * @param columnNames the upsert key column names for this argument
             */
            public Builder upsertKey(String... columnNames) {
                checkNotNull(columnNames, "columnNames must not be null");
                checkArgument(columnNames.length > 0, "Must specify at least one column");
                this.upsertKey = columnNames;
                return this;
            }
    ...
    public static class Builder<OUT> {
        ...
        private final Map<String, String[]> tableArgumentUpsertKeys = new HashMap<>();
        @Nullable private ChangelogMode outputChangelogMode = null;
        ...
        public Builder<OUT> withTableArgument(TableArgument tableArgument) {
            ...
            if (tableArgument.upsertKey != null) {
                tableArgumentUpsertKeys.put(tableArgument.name, tableArgument.upsertKey);
            }
            return this;
        }
        ...
        // ---------------------------------------------------------------------
        // Changelog Mode Configuration
        // ---------------------------------------------------------------------

        /**
         * Overrides the {@link ChangelogMode} returned by {@link
         * ProcessTableFunction.Context#getChangelogMode()} for every invocation.
         *
         * <p>Optional for a {@link ChangelogFunction}-implementing PTF: if unset, the mode is
         * derived automatically. Prefer pinning it explicitly for a hint-sensitive {@link
         * ChangelogFunction} (one whose result depends on the required-mode hint it receives),
         * whose automatically derived mode can otherwise differ from production. A PTF that does
         * not implement {@link ChangelogFunction} must leave this at {@link
         * ChangelogMode#insertOnly()}, its contractual default.
         *
         * @param mode the changelog mode the function under test declares/produces
         */
        public Builder<OUT> withOutputChangelogMode(ChangelogMode mode) {
            checkNotNull(mode, "mode must not be null");
            this.outputChangelogMode = mode;
            return this;
        }
        ...
        public ProcessTableFunctionTestHarness<OUT> build() throws Exception {
            ...
            boolean isChangelogFunction = function instanceof ChangelogFunction;
            String sourceDescription =
                    outputChangelogMode != null
                            ? "explicitly configured via withOutputChangelogMode"
                            : "resolved by PtfChangelogModeResolver";
            changelogValidator.validateResolvedOutputMode(
                    effectiveOutputChangelogMode, sourceDescription, isChangelogFunction);
        ...
        private ChangelogMode resolveOutputChangelogMode(
            ...
            if (outputChangelogMode != null) {
                return outputChangelogMode;
            } else if (function instanceof ChangelogFunction) {
    ...
    static class TableArgumentInfo extends ArgumentInfo {
        ...
        @Nullable final String[] upsertKey;
        ...
        /**
         * A null {@link #changelogMode} means the argument was not explicitly configured, which is
         * contractually insert-only.
         */
