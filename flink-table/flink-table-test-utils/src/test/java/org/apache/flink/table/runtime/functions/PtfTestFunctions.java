...
class PtfTestFunctions {
    ...
    /**
     * Marker interface for PTFs whose output changelog mode is always pinned via {@code
     * withOutputChangelogMode(...)} in the test. The default {@link ChangelogFunction} stub throws
     * so any accidental invocation by the resolver surfaces immediately.
     */
    interface UnusedChangelogMode extends ChangelogFunction {
        @Override
        default ChangelogMode getChangelogMode(ChangelogContext changelogContext) {
            throw new UnsupportedOperationException("Not invoked by the harness.");
        }
    }
    ...
    public static class UpdatingPassthroughPTF extends ProcessTableFunction<Row>
            implements UnusedChangelogMode {
        public void eval(
                @ArgumentHint({
    ...
    /**
     * Echoes the resolved output mode, whether pinned via {@code withOutputChangelogMode()} or
     * derived by the resolver, in which case its own {@code getChangelogMode()} ties it to the
     * input.
     */
    ...
    public static class InputDrivenChangelogModePTF extends ProcessTableFunction<Row>
        ...
        @Override
        public ChangelogMode getChangelogMode(ChangelogContext changelogContext) {
            ChangelogMode inputMode = changelogContext.getTableChangelogMode(0);
            return inputMode == null ? ChangelogMode.insertOnly() : inputMode;
        }
    }

    /** Emits retract-style output. */
    @DataTypeHint("ROW<value INT>")
    public static class UpdatingOutputPTF extends ProcessTableFunction<Row>
        ...
        public void eval(Context ctx, @ArgumentHint(ArgumentTrait.ROW_SEMANTIC_TABLE) Row input) {
            ...
            collect(Row.ofKind(RowKind.UPDATE_BEFORE, v));
            collect(Row.ofKind(RowKind.UPDATE_AFTER, v + 1));
        }
    }
